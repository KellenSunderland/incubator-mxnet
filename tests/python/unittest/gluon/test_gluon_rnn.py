...
class TestGluonRnn:
    ...
    def test_rnn(self):
        ...
        assert sorted(cell.collect_params().keys()) == ['rnn_h2h_bias', 'rnn_h2h_weight', 'rnn_i2h_bias',
                                                        'rnn_i2h_weight']
    ...
    @attr('cpu')
    @attr('gpu')
    def test_residual_bidirectional(self):
    ...
    def test_stack(self):
        ...
        assert outputs.list_outputs() == ['rnn_stack4_t0_out_output', 'rnn_stack4_t1_out_output',
                                          'rnn_stack4_t2_out_output']
    ...
    @attr('cpu')
    @attr('gpu')
    def check_rnn_forward(self, layer, inputs, deterministic=True):
        inputs.attach_grad()
        layer.collect_params().initialize()
        with mx.autograd.record():
    ...
    def test_rnn_cells(self):
        self.check_rnn_forward(gluon.rnn.LSTMCell(100, input_size=200), mx.nd.ones((8, 3, 200)))
        self.check_rnn_forward(gluon.rnn.RNNCell(100, input_size=200), mx.nd.ones((8, 3, 200)))
        self.check_rnn_forward(gluon.rnn.GRUCell(100, input_size=200), mx.nd.ones((8, 3, 200)))

        bilayer = gluon.rnn.BidirectionalCell(gluon.rnn.LSTMCell(100, input_size=200),
                                              gluon.rnn.LSTMCell(100, input_size=200))
        self.check_rnn_forward(bilayer, mx.nd.ones((8, 3, 200)))

        self.check_rnn_forward(gluon.rnn.DropoutCell(0.5), mx.nd.ones((8, 3, 200)), False)

        self.check_rnn_forward(gluon.rnn.ZoneoutCell(gluon.rnn.LSTMCell(100, input_size=200),
                                                     0.5, 0.2),
        ...
        self.check_rnn_forward(net, mx.nd.ones((8, 3, 200)))

    @attr('cpu')
    @attr('gpu')
    def test_rnn_layers(self):
        self.check_rnn_layer_forward(gluon.rnn.RNN(10, 2), mx.nd.ones((8, 3, 20)))
        self.check_rnn_layer_forward(gluon.rnn.RNN(10, 2), mx.nd.ones((8, 3, 20)), mx.nd.ones((2, 3, 10)))
        self.check_rnn_layer_forward(gluon.rnn.LSTM(10, 2), mx.nd.ones((8, 3, 20)))
        self.check_rnn_layer_forward(gluon.rnn.LSTM(10, 2), mx.nd.ones((8, 3, 20)),
                                     [mx.nd.ones((2, 3, 10)), mx.nd.ones((2, 3, 10))])
        self.check_rnn_layer_forward(gluon.rnn.GRU(10, 2), mx.nd.ones((8, 3, 20)))
        self.check_rnn_layer_forward(gluon.rnn.GRU(10, 2), mx.nd.ones((8, 3, 20)), mx.nd.ones((2, 3, 10)))
    ...
    @attr('cpu')
    @attr('gpu')
    def test_cell_fill_shape(self):
        cell = gluon.rnn.LSTMCell(10)
        cell.hybridize()
        self.check_rnn_forward(cell, mx.nd.ones((2, 3, 7)))
        assert cell.i2h_weight.shape[1] == 7, cell.i2h_weight.shape[1]

    @attr('cpu')
    @attr('gpu')
    def test_layer_fill_shape(self):
        layer = gluon.rnn.LSTM(10)
        layer.hybridize()
        self.check_rnn_layer_forward(layer, mx.nd.ones((3, 2, 7)))
        print(layer)
