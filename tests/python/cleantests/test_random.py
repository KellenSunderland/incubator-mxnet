...
class TestRandom:
    ...
    def check_with_device(device, dtype):
        ...
        for symbdic in symbols:
            ...
            assert device.device_type == 'gpu' or TestRandom.same(ret1, ret2), \
            ...
            assert TestRandom.same(xgrad.asnumpy(), un1.asnumpy())
            mx.random.seed(128)
            yexec.forward()
            un2 = (yexec.outputs[0] - x).copyto(device)
            assert device.device_type == 'gpu' or TestRandom.same(un1.asnumpy(), un2.asnumpy()), \
    ...
    def test_random(self):
        TestRandom.check_with_device(mx.context.current_context(), 'float16')
        TestRandom.check_with_device(mx.context.current_context(), 'float32')
        TestRandom.check_with_device(mx.context.current_context(), 'float64')
