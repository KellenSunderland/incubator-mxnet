...
class TestAutograd:
    ...
    def grad(func, argnum=None):
        ...
        """
        grad_with_loss_func = TestAutograd.grad_and_loss(func, argnum)
    ...
    def autograd_assert(*args, **kwargs):
        ...
        grad_func = TestAutograd.grad_and_loss(func, argnum)
    ...
    def test_unary_func(self):
        def check_unary_func(x):
            ...
            TestAutograd.autograd_assert(x, func=f_exp, grad_func=f_exp_grad)
            f_half = lambda x: x / 2
            f_half_grad = lambda x: [nd.ones(x.shape) * 0.5]
            TestAutograd.autograd_assert(x, func=f_half, grad_func=f_half_grad)
            f_square = lambda x: x ** 2
            f_square_grad = lambda x: [2 * x]
            TestAutograd.autograd_assert(x, func=f_square, grad_func=f_square_grad)
    ...
    def test_binary_func(self):
        def check_binary_func(x, y):
            ...
            TestAutograd.autograd_assert(x, y, func=f_add, grad_func=f_add_grad)
            f_mul = lambda x, y: x * y
            f_mul_grad = lambda x, y: [y, x]
            TestAutograd.autograd_assert(x, y, func=f_mul, grad_func=f_mul_grad)
            f_compose = lambda x, y: x + x * y
            f_compose_grad = lambda x, y: [nd.ones(x.shape) + y, x]
            TestAutograd.autograd_assert(x, y, func=f_compose, grad_func=f_compose_grad)
    ...
    def test_operator_with_state(self):
        ...
        grad_func = TestAutograd.grad_and_loss(f_fc)
    ...
    def test_argnum(self):
        ...
        TestAutograd.autograd_assert(a, b, True,
                        argnum=[0, 1], func=f_with_mode, grad_func=f_add_grad)
        TestAutograd.autograd_assert(a, b, False,
