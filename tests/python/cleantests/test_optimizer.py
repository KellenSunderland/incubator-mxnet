...
class PySGD(mx.optimizer.Optimizer):
    """python reference implementation of sgd"""
...
class TestOptimizer:
    ...
    @attr('cpu')
    @raises(UserWarning)
    def test_learning_rate_expect_user_warning(self):
        lr_s = lr_scheduler.FactorScheduler(step=1)
    ...
    def test_lr_wd_mult(self):
        ...
        mod.forward(mx.io.DataBatch(data=[mx.random.uniform(low=-1.0, high=1.0, shape=(5, 10))], label=None),
                    is_train=True)
        ...
        if t1 is not None and t2 is not None:
            if isinstance(t1, tuple):
                for s1, s2 in zip(t1, t2):
                    TestOptimizer.compare_ndarray_tuple(s1, s2, rtol, atol)
            else:
    ...
    def compare_optimizer(opt1, opt2, shape, dtype, w_stype='default', g_stype='default'):
        ...
        TestOptimizer.compare_ndarray_tuple(state1, state2)

        opt1.update_multi_precision(0, w1, g1, state1)
        opt2.update_multi_precision(0, w2, g2, state2)
        TestOptimizer.compare_ndarray_tuple(state1, state2, rtol=1e-4, atol=1e-5)
                        ...
                        for wd_option in wd_options:
                            for mp_option in mp_options:
                                ...
                                if (dtype == np.float16 and
                                    ...
                                    continue
                                TestOptimizer.compare_optimizer(opt1(**kwarg), opt2(**kwarg), shape, dtype)
                                # test operator fallback on cpu
                                if default_context() == mx.cpu():
                                    TestOptimizer.compare_optimizer(opt1(**kwarg), opt2(**kwarg), shape, dtype,
                                                                    g_stype='row_sparse')
                                    if dtype != np.float16:
                                        TestOptimizer.compare_optimizer(opt1(**kwarg), opt2(**kwarg), shape[:2],
                                                                        dtype, w_stype='csr', g_stype='csr')
        # test optimizer with a big shape
        big_shape = (54686454, 1)
        kwarg = {'momentum': 0.9, 'wd': 0.05}
        TestOptimizer.compare_optimizer(opt1(**kwarg), opt2(**kwarg), big_shape, np.float32)
                ...
                for rg_option in rg_options:
                    for wd_option in wd_options:
                        for mp_option in mp_options:
                            ...
                            TestOptimizer.compare_optimizer(opt1(**kwarg), opt2(**kwarg), shape, dtype)
                            if (default_context() == mx.cpu()):
                                TestOptimizer.compare_optimizer(opt1(sparse_update=True, **kwarg), opt2(**kwarg), shape,
                                                                dtype, w_stype='row_sparse', g_stype='row_sparse')

    @attr('gpu')
    @attr('cpu')
    @unittest.skip(
        "Test fails intermittently. Temporarily disabled until fixed. Tracked at https://github.com/apache/incubator-mxnet/issues/8230")
    def test_rms(self):
        mx.random.seed(0)
        opt1 = PyRMSProp
    ...
    def test_ftrl(self):
        ...
        for kwarg in kwargs:
            TestOptimizer.compare_optimizer(opt1(**kwarg), opt2(**kwarg), shape, np.float32)
            TestOptimizer.compare_optimizer(opt1(sparse_update=True, **kwarg), opt2(**kwarg), shape,
